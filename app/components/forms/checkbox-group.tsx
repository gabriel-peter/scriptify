export default function CheckboxGroup({ label, options }: { label: string, options: string[] }) {
    return (
        <fieldset>
                ...
                {options.map((object, idx) => (
                    <div key={idx} className="relative flex items-start py-4">
                        <div className="min-w-0 flex-1 text-sm leading-6">
                            <label htmlFor={`condition-${object}`} className="select-none font-medium text-gray-900">
                                {object}
                            </label>
                        </div>
